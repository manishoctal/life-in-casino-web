...
function BidDetail() {
    ...
    if (details?.fancyType == "diamond") {
      ...
      const coinListener = (message) => {
        if (message?.eventId === parmas?.eventId) {
          ...
          // setFancyCategoryTabArray(message.results);
  ...
  useEffect(() => {
    if (
      ...
      const connect = () => {
        ...
        if (fancyCentralizedIdArray?.length > 0) {
  ...
  }, [matchCentralizedIds, fancyCentralizedIdArray]);
  const getFancyList = async () => {
    const { status, data: response_users } = await apiGet(
      apiPath.fancyList + `?eventId=${parmas.eventId}`
    );
    if (status === 200) {
      if (response_users.success) {
        if (details.fancyType !== "diamond") {
          ...
          // setFancyCategoryTabArray(response_users?.results);
          const fancyCentralizedIdArr = response_users?.results?.map(
            (rt) => rt.centralizedId
          );
          fancyCentralizedIdArr?.length &&
            setFancyCentralizedIdArray(fancyCentralizedIdArr);
  ...
  useEffect(() => {
    if (!isEmpty(user)) {
      ...
      const updateFancySocket = (e) => {
        // console.log(e.eventIds.includes(parmas.eventId), "============");
        if (
          e.eventIds.includes(parmas.eventId) &&
      ...
      newSocket.on("updateFancySocket", updateFancySocket);
  ...
  // console.log(fancyCategoryTabArray,"===========")
  return (
    <>
